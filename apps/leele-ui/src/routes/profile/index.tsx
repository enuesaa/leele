...
export function Page() {
  ...
  if (isLoading) return <div className='mx-auto max-w-5xl px-6 py-10 text-[#1a1a1a]'>Loading...</div>
      ...
      <div className='flex flex-col items-center gap-6 rounded-2xl border border-[#ccc] bg-white/30 p-8'>
        {user?.picture && (
          <img
            src={user.picture}
            alt={user?.name}
            className='h-24 w-24 rounded-full border border-[#ccc] object-cover'
          />
        )}
        ...
        <button
          ...
          className='rounded-md border border-[#bbb] px-4 py-2 text-sm text-[#1a1a1a] transition-colors hover:bg-[#bbb]'
        >
          ログアウト
        </button>
      </div>
